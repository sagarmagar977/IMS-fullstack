...
def build_low_stock_alert_subject(stock):
    return f"Low stock alert: {stock.item.title}"


def build_low_stock_alert_message(stock):
    return (
        ...
        f"Category: {stock.item.category.name}\n"
    )


def send_low_stock_alert_for_stock(stock, trigger):
    subject = build_low_stock_alert_subject(stock)
    message = build_low_stock_alert_message(stock)
    metadata = {
