...
from .models import AssetType, ConsumableStock, ConsumableStockTransaction, FixedAsset, InventoryItem, InventoryItemType
...
class AssetTypeViewSet(viewsets.ReadOnlyModelViewSet):
    queryset = AssetType.objects.filter(is_active=True).order_by("id")
...
class InventoryItemViewSet(viewsets.ModelViewSet):
    ...
    @action(detail=False, methods=["get"], url_path="item-type-choices")
    def item_type_choices(self, request):
        choices = [
            {"value": value, "label": label}
            for value, label in InventoryItemType.choices
        ]
