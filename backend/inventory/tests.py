...
class InventoryAccessAndTransactionTests(APITestCase):
    ...
    def test_item_type_choices_endpoint_returns_two_types(self):
        self.client.force_authenticate(user=self.provincial_user)
        response = self.client.get("/api/inventory-items/item-type-choices/")
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        values = {choice["value"] for choice in response.data}
        self.assertEqual(values, {"FIXED_ASSET", "CONSUMABLE"})

    def test_asset_types_endpoint_returns_seeded_db_types(self):
        self.client.force_authenticate(user=self.provincial_user)
        response = self.client.get("/api/asset-types/")
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        values = {choice["code"] for choice in response.data["results"]}
        self.assertEqual(values, {"FIXED_ASSET", "CONSUMABLE"})
